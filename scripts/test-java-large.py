...
def modify_image():
    #Hellow.java.new has 1000 extra lines in the format get("/#", (req, res) -> {return "hello from sparkjava.com";});
    shutil.copyfile("Hello.java.new", "src/main/java/sparkexample/Hello.java")
...
def get_build_time(image_name, changed_step):
    ...
    while p.poll() is None:
        ...
        if pattern.match(line):
            if line.split("/")[0][-1] == str(changed_step+1):
...
def sync_files(srcdir, destdir):
    shutil.copyfile(os.path.join(srcdir, "Hello.java.new"),os.path.join(destdir, "Hello.java")) 
...
def inject_code(image_tar_path, changed_step):
    ...
    with open("Dockerfile") as f:
        ...
        for line in f.readlines():
            if line.strip():
                step_count += 1
                if (step_count > changed_step) and ("COPY" in line.upper() or "ADD" in line.upper()) and line[0] != "#":
    ...
    sync_files("./", "{}/code/src/main/java/sparkexample".format(changed_layer_dir))
...
def clean_up(image_name):
    ...
    shutil.copyfile("Hello.java", "src/main/java/sparkexample/Hello.java")
...
def test_one(args):
    ...
    modify_image()
    t = get_build_time(args.image+"2", args.changed_step)
    ...
    t = get_code_injection_time(os.path.join("temp", saved_as), args.changed_step) + get_build_time(args.image+"2", args.changed_step)
